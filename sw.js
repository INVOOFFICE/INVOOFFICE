...
const CACHE = 'invooffice-landing-a1eb818dd1'; /* remplacé automatiquement au build (bump-sw-cache.mjs) */
...
const REL_ASSETS = [
  ...
  'images/INVOOFFICE.png',
  'images/INVOORENT.png',
  'images/INVOOCOFFEE.png',
  'images/pwa-192.png',
  'images/pwa-512.png',
];
  ...
  event.waitUntil(
    caches.open(CACHE).then((cache) =>
      Promise.all(
        [
          scopedUrl('./'),
          ...REL_ASSETS.map((rel) => scopedUrl(rel)),
        ].map((url) =>
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    caches.match(request).then((hit) => {
      ...
      const pathOnly = sameOriginPathOnly(request);
      if (pathOnly && pathOnly !== request.url) {
        return caches.match(pathOnly).then((byPath) => {
